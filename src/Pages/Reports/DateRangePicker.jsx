...
const DateRangePicker = ({ dateRange, setDateRange, onSearch }) => {
  ...
  const handleDateClick = (date) => {
    ...
    if (selectedDates.length === 0) {
      ...
      if (firstDateMidnight.getTime() === clickedDateMidnight.getTime()) {
        ...
        start = firstDateMidnight < clickedDateMidnight ? firstDateMidnight : clickedDateMidnight;
        start.setHours(0, 0, 0, 0);  // Start of earlier date (12:00 AM)
        
        end = firstDateMidnight > clickedDateMidnight ? firstDateMidnight : clickedDateMidnight;
  ...
  return (
    <div className="relative mb-3" ref={calendarRef}>
      <div className="flex border-gray-200 border rounded-full gap-3">
        <button
          onClick={() => setShowCalendar(!showCalendar)}
          className="flex-1 text-sm px-4 py-3 bg-white rounded-full text-left text-gray-400 hover:bg-gray-200 transition-colors"
        >
          {formatDateRangeDisplay() || "DD / MM / YYYY - DD / MM / YYYY"}
        </button>
        <button
          onClick={onSearch}
          className="px-4 py-3 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors"
        >
